...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def build_ui(self):
        ...
        self.lbl_temp = QtWidgets.QLabel("0.0")
        f_temp = QtGui.QFont()
        ...
        self.lbl_temp.setStyleSheet(
            "color: #1D4ED8; font-size: 60pt; font-weight: 800;"  # Reduced from 72pt
        )
        ...
        # °C floating overlay (child of temp_box, positioned bottom-right)
        self.lbl_unit = QtWidgets.QLabel("°C", temp_box)
        self.lbl_unit.setStyleSheet(
            "color: #1D4ED8; font-size: 50pt; font-weight: 800; background: transparent;"
        )
        self.lbl_unit.adjustSize()
        self.lbl_unit.raise_()

        # Keep °C at bottom-right on resize
        original_resize = temp_box.resizeEvent
        def _reposition_unit(event):
            if original_resize:
                original_resize(event)
            self.lbl_unit.move(
                temp_box.width() - self.lbl_unit.width() - 12,
                20
            )
        temp_box.resizeEvent = _reposition_unit
    ...
    def handle_new_temp(self, temp_raw: float):
        ...
        self.lbl_temp.setText(f"{temp_adj:.1f}")
        ...
        if temp_adj >= self.over_temp_threshold:
            ...
            # Temperature back to normal - but DO NOT cancel emergency sequence
            # Once triggered, emergency runs to completion (buzzer 10s -> relay ON)
            # Only Reset button can turn off relay
            if not self.buzzer_triggered:
                # Only show NORMAL if no emergency was triggered
            ...
            # NOTE: Relay stays ON (latched) until manually turned off via EMG MANUAL button
            # This prevents relay from toggling ON/OFF repeatedly as temperature fluctuates
            # User must manually reset the emergency state
